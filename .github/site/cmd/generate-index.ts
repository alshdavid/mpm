...
export async function main() {
    ...
    for (const model of await fs.readdir(Paths["~/models/"](modelType))) {
      promises.push(
        new Promise((res) => setTimeout(res, 0)).then(async () => {
          ...
          if (!modelVersion) {
            throw new Error(
              `Unable to find ${model}:${config.model_version_id}`
            );
          }
          ...
          manifest[modelName] = {
            download_url: modelFile.downloadUrl,
            source: "civitai",
            kind: yaml.type,
            format: yaml.format,
          };
  ...
  await fs.writeFile(
    ...
    JSON.stringify(
      Object.keys(versionManifest).sort(sortEntries).reverse(),
      null,
      2
    ),
    "utf8"
